    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <h4 class="fw-bold m-0">Statuses</h4>
            </div>
        </div>
        
        @can('status_create')
        <a href="{{ route('admin.statuses.create') }}" class="btn btn-primary d-flex align-items-center gap-2 shadow-sm rounded-pill px-4">
    ...
    <div class="card table-card">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle w-100 mb-0" id="statusesTable">
                        ...
                        @foreach($statuses as $key => $status)
                            <tr data-entry-id="{{ $status->id }}">
                                <td class="text-center ps-4">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input dt-checkboxes" value="{{ $status->id }}">
                                    </div>
                                </td>
                                <td class="ps-3 fw-bold text-muted">#{{ $status->id }}</td>
                                <td>
                                    <!-- Visual Preview Nama Status dengan Warna Background -->
                                    <span class="badge rounded-pill border" 
                                          style="background-color: {{ $status->color }}; color: #fff; text-shadow: 0 1px 2px rgba(0,0,0,0.4); font-weight: 500; font-size: 0.85rem; padding: 6px 12px;">
                                        {{ $status->name }}
                                    </span>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <span class="color-dot shadow-sm" style="background-color: {{ $status->color }}"></span>
                                        <span class="font-monospace text-muted small">{{ $status->color ?? '#FFFFFF' }}</span>
                                    </div>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        @can('status_show')
                                            <a href="{{ route('admin.statuses.show', $status->id) }}" class="btn-icon view" data-bs-toggle="tooltip" title="View">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                        @endcan

                                        @can('status_edit')
                                            <a href="{{ route('admin.statuses.edit', $status->id) }}" class="btn-icon edit" data-bs-toggle="tooltip" title="Edit">
                                                <i class="bi bi-pencil"></i>
                                            </a>
                                        @endcan

                                        @can('status_delete')
                                            <form action="{{ route('admin.statuses.destroy', $status->id) }}" method="POST" class="d-inline delete-form">
...
<script>
    $(function () {
        ...
        let table = $('#statusesTable').DataTable({
            language: {
                search: "",
        ...
        $('#massDeleteBtn').on('click', function() {
            ...
            if (ids.length > 0) {
                Swal.fire({
                    title: 'Are you sure?',
                    text: `You are about to delete ${ids.length} statuses. Use caution!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        .done(function () {
                            Swal.fire('Deleted!', 'Selected statuses have been deleted.', 'success').then(() => {
                                location.reload();
                            });
                        });
                    }
        ...
        $(document).on('submit', '.delete-form', function(e) {
            ...
            Swal.fire({
                title: 'Delete Status?',
                text: "Tickets with this status might lose their status definition.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ef4444',
                cancelButtonColor: '#94a3b8',

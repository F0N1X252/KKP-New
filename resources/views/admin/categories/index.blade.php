...
<div class="container-fluid p-0">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
        ...
        @can('category_create')
        <a href="{{ route('admin.categories.create') }}" class="btn btn-primary d-flex align-items-center gap-2 shadow-sm rounded-pill px-4">
    ...
    <div class="card table-card">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle w-100 mb-0" id="categoriesTable">
                        ...
                        @foreach($categories as $key => $category)
                            <tr data-entry-id="{{ $category->id }}">
                                <td class="text-center ps-4">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input dt-checkboxes" value="{{ $category->id }}">
                                    </div>
                                </td>
                                <td class="ps-3 fw-bold text-muted">#{{ $category->id }}</td>
                                <td>
                                    <!-- Visual Preview Nama Category dengan Warna Background -->
                                    <span class="badge rounded-pill border" 
                                          style="background-color: {{ $category->color }}; color: #fff; text-shadow: 0 1px 2px rgba(0,0,0,0.4); font-weight: 500; font-size: 0.85rem; padding: 6px 12px;">
                                        {{ $category->name }}
                                    </span>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <span class="color-dot shadow-sm" style="background-color: {{ $category->color }}"></span>
                                        <span class="font-monospace text-muted small">{{ $category->color ?? '#FFFFFF' }}</span>
                                    </div>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end gap-1">
                                        @can('category_show')
                                            <a href="{{ route('admin.categories.show', $category->id) }}" class="btn-icon view" data-bs-toggle="tooltip" title="View">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                        @endcan

                                        @can('category_edit')
                                            <a href="{{ route('admin.categories.edit', $category->id) }}" class="btn-icon edit" data-bs-toggle="tooltip" title="Edit">
    ...
    $(function () {
        ...
        $('#massDeleteBtn').on('click', function() {
            ...
            if (ids.length > 0) {
                Swal.fire({
                    title: 'Are you sure?',
                    text: `You are about to delete ${ids.length} categories. This cannot be undone!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        .done(function () {
                            Swal.fire('Deleted!', 'Selected categories have been deleted.', 'success').then(() => {
                                location.reload();
                            });
                        });
                    }
        ...
        $(document).on('submit', '.delete-form', function(e) {
            ...
            Swal.fire({
                ...
                text: "Tickets in this category might lose their categorization.",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#ef4444',
                cancelButtonColor: '#94a3b8',

...
<div class="container-fluid p-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h4 class="fw-bold m-0">Edit Category</h4>
            ...
            <form action="{{ route("admin.categories.update", [$category->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-8">
                        <!-- Name Input -->
                        <div class="mb-4">
                            <label for="name" class="form-label fw-bold required">{{ trans('cruds.category.fields.name') }}</label>
                            <input type="text" id="name" name="name" class="form-control form-control-lg {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name', $category->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <!-- Color Input -->
                        <div class="mb-4">
                            <label for="color" class="form-label fw-bold">{{ trans('cruds.category.fields.color') }}</label>
                            <div class="input-group">
                                <input type="text" id="color" name="color" class="form-control form-control-lg colorpicker {{ $errors->has('color') ? 'is-invalid' : '' }}" value="{{ old('color', $category->color) }}">
                                <span class="input-group-text color-preview-addon">
                                    <!-- Preview box langsung menggunakan warna dari database -->
                                    <i class="color-preview-box" style="background-color: {{ old('color', $category->color ?? '#FFFFFF') }};"></i>
                                </span>
                            </div>
                            @if($errors->has('color'))

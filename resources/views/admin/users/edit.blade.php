    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="text-muted small">Update user information and assign roles</div>
            </div>
        </div>
        ...
        <div class="card-body p-4">
            <form action="{{ route("admin.users.update", [$user->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="name" name="name" class="form-control form-control-lg {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name', $user->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="mb-4">
                    ...
                    <input type="email" id="email" name="email" class="form-control form-control-lg {{ $errors->has('email') ? 'is-invalid' : '' }}" value="{{ old('email', $user->email) }}" required>
                    @if($errors->has('email'))
                        <div class="invalid-feedback">
                            {{ $errors->first('email') }}
                        </div>
                ...
                <!-- Password Input -->
                <div class="mb-4">
                    <label for="password" class="form-label fw-bold">{{ trans('cruds.user.fields.password') }}</label>
                    ...
                    @endif
                    <div class="form-text text-muted">
                        <i class="bi bi-info-circle me-1"></i> {{ trans('cruds.user.fields.password_helper') }} (Leave blank to keep current password)
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        // Mencari elemen select dengan ID roles, set semua option jadi selected, lalu trigger change
        ...
        // Menghapus status selected dari semua option, lalu trigger change
